...
class makeBiomass():
    ...
    def RandomBiomass(self):
        ...
        # PROTlist.append(PROTsyn[0])
        # DNAlist.append(DNAsyn)
        # RNAlist.append(RNAsyn)
        ...
        with ExcelWriter(self.file2save, mode='w', engine='openpyxl') as writer:
            ...
            pd.DataFrame(LIPID_sensitivity[1]).to_excel(writer, sheet_name='FATTYACIDsyn')
            pd.DataFrame(LIPID_sensitivity[2]).to_excel(writer, sheet_name="FATTYACIDCOAsyn")
            ...
            pd.DataFrame(BIOMASSlist).to_excel(writer, sheet_name='biomass_cho')
            writer.save()
    ...
    def PROTEIN(self,wt_per_DCW):
        ...
        AApd = pd.read_excel(self.filename, sheet_name="PROTsyn", usecols="J:AC",convert_float=True)
        ...
        for aa_i in range(20):
            ...
            for min_or_max in ["min","max"]:
                ...
                minmax_col="aacoeff_" + str(AApd['g/g Prot'][aa_i]) + "_" + str(min_or_max)
    ...
    def DNA_or_RNA(self,wt_per_DCW, DNA_or_RNA):
        ...
        if DNA_or_RNA == "RNA":
            ...
            RNA_comp_pd = pd.read_excel(self.filename, sheet_name='RNAsyn', usecols="A:G", skiprows=10 ,convert_float=True)
            RNA_comp_pd = RNA_comp_pd.iloc[:5]
            RNA_comp_pd["mean"] = RNA_comp_pd.loc[:, "iCHO1766":"Ave"].mean(axis=1)
            RNA_comp_pd.at[4, "mean"] = RNA_comp_pd["mean"][:4].sum(axis=0)
            RNA_comp_pd["mean"] = RNA_comp_pd["mean"] / RNA_comp_pd["mean"][4]

            max_cv = 0.25
            RNA_comp_pd["min"] = RNA_comp_pd["mean"][:5] - (max_cv * RNA_comp_pd["mean"][:5])
            RNA_comp_pd["max"] = RNA_comp_pd["mean"][:5] + (max_cv * RNA_comp_pd["mean"][:5])


            #ref
            RNA_comp_pd["g/g DNA"].at[4] = RNA_comp_pd["g/g DNA"][:4].sum()
            recal_RNA_ref = RNA_comp_pd.filter(like="g/g DNA", axis=1)
            ...
            for RNA_i in range(4):
                ...
                for min_or_max in ["min", "max"]:
                    ...
                    minmax_col = "RNAcoeff_" + str(RNA_comp_pd["Unnamed: 0"][RNA_i]) + "_" + str(min_or_max)
                    RNA_comp_pd[minmax_col] = RNA_norm
                    RNA_comp_pd.at[4, minmax_col] = RNA_comp_pd[minmax_col][:4].sum()
                    RNA_comp_pd["gg_cal"] = RNA_norm
                    for gg_i in range(4):
                        RNA_comp_pd["gg_cal"].iloc[gg_i]  = RNA_comp_pd[minmax_col].iloc[gg_i] * RNA_comp_pd["MW (g/mol)"].iloc[gg_i]
                    RNA_comp_pd["gg_cal"].at[4] = RNA_comp_pd["gg_cal"][:4].sum(axis=0)
                    RNA_comp_pd[minmax_col].iloc[:4] = RNA_comp_pd["gg_cal"].iloc[:4] / RNA_comp_pd[minmax_col].iloc[4]
                    RNA_comp_pd[minmax_col].at[4] = RNA_comp_pd[minmax_col][:4].sum(axis=0)
                    RNA_comp_pd[minmax_col].iloc[:4] = RNA_comp_pd[minmax_col].iloc[:4] / RNA_comp_pd[minmax_col][4]
            ...
            DNA_comp_pd = pd.read_excel(self.filename, sheet_name='DNAsyn', usecols="A:G", skiprows=10 ,convert_float=True)
            DNA_comp_pd = DNA_comp_pd.iloc[:5]
            DNA_comp_pd["mean"] = DNA_comp_pd.loc[:, "iCHO1766)":"Average mol %"].mean(axis=1)
            DNA_comp_pd.at[4, "mean"] = DNA_comp_pd["mean"][:4].sum(axis=0)
            DNA_comp_pd["mean"] = DNA_comp_pd["mean"] / DNA_comp_pd["mean"][4]

            DNA_comp_pd["min"] = DNA_comp_pd["mean"][:5] - (0.25 * DNA_comp_pd["mean"][:5])
            DNA_comp_pd["max"] = DNA_comp_pd["mean"][:5] + (0.25 * DNA_comp_pd["mean"][:5])
            ...
            #ref
            DNA_comp_pd["g/g DNA"].at[4] = DNA_comp_pd["g/g DNA"][:4].sum()
            recal_DNA_ref = DNA_comp_pd.filter(like="g/g DNA", axis=1)
            ...
            for DNA_i in range(4):
                ...
                for min_or_max in ["min", "max"]:
                    ...
                    minmax_col = "DNAcoeff_" + str(DNA_comp_pd["Unnamed: 0"][DNA_i]) + "_" + str(min_or_max)
                    DNA_comp_pd[minmax_col] = DNA_norm
                    DNA_comp_pd.at[4, minmax_col] = DNA_comp_pd[minmax_col][:4].sum()
                    DNA_comp_pd["gg_cal"] = DNA_norm
                    for gg_i in range(4):
                        DNA_comp_pd["gg_cal"].iloc[gg_i]  = DNA_comp_pd[minmax_col].iloc[gg_i] * DNA_comp_pd["MW (g/mol)"].iloc[gg_i]
                    DNA_comp_pd["gg_cal"].at[4] = DNA_comp_pd["gg_cal"][:4].sum(axis=0)
                    DNA_comp_pd[minmax_col].iloc[:4] = DNA_comp_pd["gg_cal"].iloc[:4] / DNA_comp_pd[minmax_col].iloc[4]
                    DNA_comp_pd[minmax_col].at[4] = DNA_comp_pd[minmax_col][:4].sum(axis=0)
                    DNA_comp_pd[minmax_col].iloc[:4] = DNA_comp_pd[minmax_col].iloc[:4] / DNA_comp_pd[minmax_col][4]
    ...
    def LIPID(self,LIPID_wt_per_DCW):
        ...
        LIPIDin = self.processBiomassFrame(LIPIDpd,-1)
        ...
        FAcoain = self.processBiomassFrame(FAcoapd, -1)
        ...
        max_cv = 0.25
        FA_minmax_pd["min"] = FA_minmax_pd["norm_mean"] * (1 - max_cv)
    ...
    def BIOMASS(self):
        ...
        BIOMASSin = BIOMASSpd.iloc[:7, 0:5]
